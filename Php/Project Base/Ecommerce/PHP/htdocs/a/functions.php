...
Copyright (c) 2011 Liberty Reserve.
...
class Transfer {
	...
	function Transfer($payeeAcct, $amount, $memo = '') {
...
class TransferRequest {
  ...
  var $payerAcct;	
	...
	function TransferRequest($payerAcct, $apiName, $secWord) {
	...
	function toXml() {
		$authPart = 
			"<Auth><AccountId>".$this->payerAcct."</AccountId><ApiName>".$this->apiName."</ApiName><Token>".$this->authToken."</Token></Auth>";
	...
	function addTransfersFromText($transferList, &$errors) {
		$lines = explode("\n", $transferList);
		...
		for ($i = 0; $i < count($lines); $i++) {
			...
			if (count($parts) >=3  && 
				...
				$trans = new Transfer(trim($parts[0]), trim($parts[1]));
	...
	function getResponse() {
		...
		$fields_string = "req=". urlencode($this->toXml());
    
    $size = strlen($fields_string);
    if($size > 8000)  {
      die("Api request max length exceeded. Maximum request size is 6100KB. The current request size is $size bytes.");
      return;
    }     
		...
		curl_setopt($handler, CURLOPT_SSL_VERIFYPEER, 0); // ignore SSL certificate
    ...
    curl_setopt($handler, CURLOPT_POSTFIELDS,$fields_string);
		...
		ob_start();
		curl_exec($handler);
		//echo $info = curl_getinfo($handler, CURLINFO_HEADER_SIZE);
